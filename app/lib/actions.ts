...
export async function createComponent(formData: FormData) {
    ...
    const { category_id, brand_id, model, image_url, actual_width, stem_x, stem_y, saddle_x, saddle_y, front_wheel_x, front_wheel_y,
        ...
        best_aerodynamics, best_lightweight, groupset_drivetrain_x, groupset_drivetrain_y, groupset_shifter_x, groupset_shifter_y } = formDataObject;
    ...
    try {
        ...
        back_wheel_x, back_wheel_y, has_stem, has_handle_bar, price, key_metrics, aerodynamics, weight, comfort, stiffness, overall, best_aerodynamics, best_lightweight, groupset_drivetrain_x, groupset_drivetrain_y, groupset_shifter_x, groupset_shifter_y)
        ...
        ${back_wheel_x}, ${back_wheel_y}, ${!!has_stem}, ${!!has_handle_bar}, ${price}, ${key_metrics}, ${aerodynamics}, ${weight}, ${comfort}, ${stiffness}, ${overall}, 
        ${best_aerodynamics}, ${best_lightweight}, ${groupset_drivetrain_x}, ${groupset_drivetrain_y}, ${groupset_shifter_x}, ${groupset_shifter_y})
      `;
    } catch (error) {
...
export async function updateModel(id: string, formData: any) {
    ...
    try {
        ...
        best_aerodynamics = ${best_aerodynamics}, best_lightweight = ${best_lightweight}, groupset_drivetrain_x = ${groupset_drivetrain_x}, groupset_drivetrain_y = ${groupset_drivetrain_y}, groupset_shifter_x = ${groupset_shifter_x}, groupset_shifter_y = ${groupset_shifter_y}
        WHERE id = ${id};
        `
