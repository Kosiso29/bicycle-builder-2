...
export async function createModel(formData: FormData) {
    ...
    const { category_id, brand_id, model, image_url, actual_width, stem_x, stem_y, saddle_x, saddle_y, front_wheel_x, front_wheel_y,
        back_wheel_x, back_wheel_y, has_stem, has_handle_bar, price, price_gb, price_sg, price_in, key_metrics, aerodynamics, weight, comfort, stiffness, overall,
    ...
    try {
        ...
        for (const color_prop of JSON.parse(color_props)) {
            const model_id = selectedModel.rows[0]?.id;
            await sql`
                INSERT INTO colors (model_id, name, value, image_url, price, price_gb, price_sg, price_in) VALUES (${model_id}::uuid, ${color_prop.name}, ${color_prop.value}, ${color_prop.image_url}, ${color_prop.price}, ${color_prop.price_gb}, ${color_prop.price_sg}, ${color_prop.price_in});
            `
        }
...
export async function updateModel(id: string, formData: any) {
    ...
    try {
        ...
        back_wheel_x = ${back_wheel_x}, back_wheel_y = ${back_wheel_y}, has_stem = ${!!has_stem}, has_handle_bar = ${!!has_handle_bar}, price = ${price}, price_gb = ${price_gb}, price_sg = ${price_sg}, price_in = ${price_in}, key_metrics = ${key_metrics}, aerodynamics = ${aerodynamics}, weight = ${weight}, comfort = ${comfort}, stiffness = ${stiffness}, overall = ${overall}, 
        ...
        for (const color_prop of JSON.parse(color_props)) {
            await sql`
                INSERT INTO colors (model_id, name, value, image_url, price, price_gb, price_sg, price_in) VALUES (${id}::uuid, ${color_prop.name}, ${color_prop.value}, ${color_prop.image_url}, ${color_prop.price}, ${color_prop.price_gb}, ${color_prop.price_sg}, ${color_prop.price_in});
            `
        }
